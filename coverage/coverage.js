...
var controls = (function($) {
    ...
    function setDisplay(display) {
        $("body").toggleClass("show-data", display === "data");
        $("body").toggleClass("show-graph", display === "graph");
    }
    ...
    function update(state) {
        ...
        var display = state.display || "data";
        $("input[name=level][value=" + lvl + "]").get(0).checked = true;
...
function View(model) {
    ...
    this.children = [];
    if (model) {
        this.statusText = View.STATUS_FULL_NAMES[model.status];
    }
}

View.maxDesiredTests = 0; // :(
View.STATUS_FULL_NAMES = {
...
(function ($) {
    ...
    var templates = (function ($) {
        ...
        Handlebars.registerHelper('formatTime', function(time) {
            ...
            if (time < D) {
                return allowHalfs(time / H) + " MH";
            }

            if (time < W) {
                return allowHalfs(time / D) + " MD";
            }

            if (time < 6 * W) {
                return allowHalfs(time / W) + " MW";
            }

            if (time < 11 * M) {
                return allowHalfs(time / M) + " MM";
            }

            return allowHalfs(time / Y) + " MY";
        });
        
        Handlebars.registerHelper('formatTimeLong', function(time) {
            var H = 60, D = 5 * H, W = 5 * D, Y = 50 * W, M = Y / 12;
            if (time < H) {
                return "1 man-hour";
            }
            ...
            if (time < D) {
                return allowHalfs(time / H) + " man-hours";
            }

            if (time < W) {
                return allowHalfs(time / D) + " man-days";
            }

            if (time < 6 * W) {
                return allowHalfs(time / W) + " man-weeks";
            }

            if (time < 11 * M) {
                return allowHalfs(time / M) + " man-months";
            }

            return allowHalfs(time / Y) + " man-years";
        });
    ...
    $("#sort-by, #rfc2119, #algos, #idl, #assume-idl, #propdef, #review-success, #reftest-factor, #test-time, #review-time, #assume-tooling").change(function() {
        var state = controls.getState();
        refreshApp(state);
        applicationState.pushState(state);
    });
    ...
    function createBarGraph(item) {
        ...
        var widthRatio = (item.desiredTests || 0) / View.maxDesiredTests;
        var passedRatio = (item.testsPassingReview || 0) / item.desiredTests;
        var needReview = Math.min(Math.round(passedRatio * 100), 100 - percent);
        item.progressBar = {
            width: Math.round(widthRatio * 100),
            reviewed: percent,
            needReview: needReview,
            done: percent - needReview,
            todo: 100 - percent - needReview
        };
    }
    ...
    window.cover = function (_specs, _$target, _onrefreshed) {
